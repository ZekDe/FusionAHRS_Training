		...
		const float samplePeriod)
{
	...
	fusionBias->filterCoefficient = (2.0f * M_PI * CORNER_FREQUENCY)
			* fusionBias->samplePeriod;
		...
		FusionVector3 gyroscope)
{
	...
	if ((fabs(gyroscope.axis.x) > fusionBias->threshold)
			|| (fabs(gyroscope.axis.y) > fusionBias->threshold)
	...
	fusionBias->gyroscopeBias = FusionVectorAdd(fusionBias->gyroscopeBias,
			FusionVectorMultiplyScalar(gyroscope,
